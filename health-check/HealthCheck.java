...
void main(String[] args) throws InterruptedException {
  ...
  var request =
      HttpRequest.newBuilder(URI.create(args[0])).timeout(TIMEOUT).GET().build();
...
boolean executeHealthCheck(HttpClient client, HttpRequest request, int attempt) {
  try {
    ...
    IO.println("Health check attempt %s/%s could not connect.".formatted(attempt + 1, MAX_ATTEMPTS));
  }
